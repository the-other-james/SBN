...
static void CheckNet(SBN_NetInterface_t *Net)
{
    ...
    for (PeerIdx = 0; PeerIdx < Net->PeerCnt; PeerIdx++)
    {
        ...
        {
            /* TODO: make a #define */
            if (OS_TimeGetTotalSeconds(LocalTime) > OS_TimeGetTotalSeconds(PeerData->LastConnectTry) + 5)
            {
...
static SBN_Status_t PollPeer(SBN_PeerInterface_t *Peer)
{
    ...
    if (SBN_TCP_PEER_HEARTBEAT > 0 && OS_TimeGetTotalSeconds(CurrentTime) - OS_TimeGetTotalSeconds(Peer->LastSend) > SBN_TCP_PEER_HEARTBEAT)
    {
        Send(Peer, SBN_TCP_HEARTBEAT_MSG, 0, NULL);
    } /* end if */

    if (SBN_TCP_PEER_TIMEOUT > 0 && OS_TimeGetTotalSeconds(CurrentTime) - OS_TimeGetTotalSeconds(Peer->LastRecv) > SBN_TCP_PEER_TIMEOUT)
    {
